...
# 提示词版本会写入后续评测元数据。
PROMPT_VERSION = "contract-risk-v3"
...
COMMERCIAL_PROFILE = AgentProfile(
    ...
    review_scope=(
        ...
        "测试标准、整改、复验和书面确认。最低合格线："
        "分期付款且保留最终验收后尾款或质保款，可视为付款"
        "保护已覆盖；验收约定测试、整改、复验和书面确认，"
        "可视为验收保护已覆盖。不得仅因比例、期限或附件细节"
        "不够理想而报告风险。"
    ),
)
...
LEGAL_PROFILE = AgentProfile(
    ...
    review_scope=(
        ...
        "服务退出和资料移交。最低合格线：源代码或开发成果"
        "权利明确归采购方，可视为知识产权保护已覆盖；延期或"
        "质量缺陷触发责任、赔偿及采购方解除权，可视为违约"
        "保护已覆盖；约定质保期并明确响应和修复时间，可视为"
        "运维保护已覆盖。不得因为缺少违约金公式、第三方组件"
        "细目、故障等级或移交清单而追加风险。只有条款缺失、"
        "保护对象不明确，或者把核心事项留待另行协商时才报告。"
    ),
)
...
SECURITY_PROFILE = AgentProfile(
    ...
    review_scope=(
        ...
        "安全事件报告和分包方责任。最低合格线：明确数据安全"
        "或保密义务，并限制访问范围或约定合同结束后返还、"
        "删除数据，即可视为基础保护已覆盖。不得因为缺少事件"
        "报告时限、保存期限或分包细节而追加风险。只有完全缺失"
        "数据保护，或者把处理范围、保存及删除等核心事项全部"
        "留待另行协商时才报告。"
    ),
)
...
def build_system_prompt(
    ...
    return (
        ...
        "审核规则：\n"
        "1. 采用精确率优先的最低充分性标准。只报告已有文本"
        "直接证明的重大不利安排、明确不可执行的另行协商，或"
        "合同整体完全缺失的必要保护。存在疑问时不报告。\n"
        "2. 不得把最佳实践、可选增强项、缺少量化细节或措辞"
        "不够完善本身当作风险。只要条款达到专业审核范围中的"
        "最低合格线，就视为已充分覆盖，不得继续寻找缺口。\n"
        "3. 不得仅因为出现关键词就报告风险；必须区分完整"
        "条款、否定表达、同义表达和明确留待协商的空洞条款。\n"
        "4. clause_risk用于已经存在但明确不利，或核心事项"
        "全部留待协商的条款，必须引用真实clause_id，"
        "expected_clause返回null。\n"
        "5. missing_clause仅在完整合同没有实质覆盖该事项时"
        "使用，clause_id返回null，并说明expected_clause。"
        "已有空洞条款应使用clause_risk。\n"
        "6. 每个允许分类最多输出一项最重要风险，不得把同一"
        "保护事项拆分成多项，也不得同时输出该分类的条款风险"
        "和缺失风险。\n"
        "7. evidence只是候选依据，不自动证明结论。必须按照"
        ...
        "空evidence_ids。\n"
        "8. reason应说明对采购方的损害和触发条件；"
        "suggested_revision应给出可执行的采购方保护措施。\n"
        "9. 风险等级：导致全部价款失去制约、自动验收、必要"
        "保护完全缺失或核心事项全部留待协商通常为high；已有"
        "基本保护但单个关键执行条件留待协商时为low或medium。"
        "不得仅为显得谨慎而提高等级。\n"
        "10. 没有范围内风险时，显式返回空findings列表。\n\n"
