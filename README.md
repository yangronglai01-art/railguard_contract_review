...
DeepSeek模式使用Responses API，通过 `text.format` 发送命名JSON Schema。当前prompt v3采用精确率优先的最低充分性标准，并向模型提供机器可读引用白名单；本地继续校验必填字段、风险分类、条款ID和证据引用权限，再通过Precision Guardrail v1过滤最佳实践型误报并校准风险等级。所有被拒绝的引用ID继续保留用于审计。
...
已经核验并随Git归档的21例正式报告见：
[deepseek-v4-pro-v1.1-report.json](data/evaluation/reports/deepseek-v4-pro-v1.1-report.json)。本次运行在第6例遇到一次无效JSON响应，随后使用 `--resume` 从已完成的5例继续，验证了增量保存和恢复机制。

当前正式结果（21例、26项标签、prompt v2、validator v2、metrics v2）：
...
通用模型识别了全部26项人工风险，但产生77条额外风险。误报主要集中在违约责任19条、数据安全18条和运维支持18条；5项等级错误全部与运维支持有关。103条模型风险均通过引用范围校验，没有被拒绝的证据ID。这一结果明确了阶段C的优化目标：保持高召回和可追溯引用，同时显著收紧风险边界、降低误报并校准运维风险等级。
...
- Prompt v3最低充分性审核口径
- 六类风险最低合格线和风险等级校准
- Precision Guardrail v1本地高置信度门槛
- 21例开发基准风险边界回归
- 15项额外高端装备制造场景挑战测试
- 历史103项原始预测的离线护栏重放

离线重放得到TP=26、FP=0、FN=0，精确率、召回率和F1均为100%。该结果只证明护栏能够处理已观察到的误报；由于21例开发基准已参与规则设计，不能作为最终泛化成绩，也不能代替模型重新推理。完整说明见 [基础模型质量优化报告](docs/model-optimization-v3-report.md)。

2026-09-18已尝试运行Prompt v3的8例真实模型小评测，但DeepSeek返回HTTP 402 `Insufficient Balance`，未产生可用结果。余额恢复后必须先完成8例小评测，再完成21例真实开发评测；真实评测达到精确率70%、召回率85%、F1 75%后，才进入微调阶段。
...
- 该报告只保留用于观察旧协议下的模型行为；正式阶段B结论以上述21例报告为准
...
SFT标注字段、来源授权、证据约束、双人复核和冻结条件记录在 `data/training/README.md`，空白记录模板为 `data/training/sft-annotation-template-v1.json`。模板不是训练样本。
...
- 已完成阶段A的21例规则基线、阶段B的21例DeepSeek prompt v2正式评测，以及prompt v3与Precision Guardrail v1离线优化；prompt v3真实复评因DeepSeek余额不足尚未完成
...
1. DeepSeek余额恢复后，先运行Prompt v3的8例小评测，再运行21例完整开发评测，确认精确率、召回率和F1达到微调准入门槛。
2. 抽样审查被Precision Guardrail v1过滤的候选项，确认没有系统性漏检。
3. 另建与开发基准和最终测试集隔离的训练集、验证集及held-out测试集，避免评测泄漏。
4. 选择开放权重底座，先生成未微调模型的领域基线。
5. 达到准入门槛后再执行LoRA或QLoRA监督微调，并在冻结held-out测试集上比较同一底座微调前后的结果。
6. 接入真实RAG服务，并继续补充OCR、认证、审计、监控和备份能力。
