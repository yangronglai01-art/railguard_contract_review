...
- 已完成阶段A的21例规则基线和阶段B的21例DeepSeek正式评测；三个模型Agent、人工中断、引用审计、增量保存和checkpoint恢复闭环已验证
...
1. 从阶段B的77条误报中归纳难例模式，另建与最终测试集隔离的训练集和开发集，避免评测泄漏。
2. 选择开放权重底座，先生成未微调模型的领域基线。
3. 完成LoRA或QLoRA合同领域监督微调，并在冻结的held-out测试集上比较同一底座微调前后的结果。
