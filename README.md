...
项目使用固定合成评测集比较不同审核实现。三个实验阶段按顺序升级，并使用同一份测试集：

| 阶段 | 被评测系统 | 目的 |
|---|---|---|
| A | 确定性规则Agent | 建立可重复的工程基线 |
| B | 通用基础模型 | 评估模型相对规则的语义理解提升 |
| C | 监督微调模型 | 评估领域微调带来的增量效果 |
...
- 只检查关键字是否出现，无法判断知识产权、违约责任、运维服务和数据安全条款内容是否充分
- 会把“不得支付全部合同款”的否定表达误判为风险

精确率、召回率和F1使用风险类型与风险分类进行一对一匹配。风险等级、原文定位和引用质量在成功匹配后单独计分。
